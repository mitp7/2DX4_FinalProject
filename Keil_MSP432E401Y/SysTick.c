...
// Time delay using busy wait.
// This assumes 120 MHz system clock, defualt to 120000
void SysTick_Wait10ms(uint32_t delay){
  uint32_t i;
  for(i=0; i<delay; i++){
		// Clock speed = 40MHz
		// Delay = 10ms = 0.01s
    SysTick_Wait(400000);    // wait 10ms (assumes 40 MHz clock) | delay(in s) = Multiplier * 1/Clock Speed -> Multiplier = 400000
  }
}
